...
#define MAX_SIZE                    12
typedef struct{
    struct entry CookerData[MAX_SIZE];
    unsigned int Head;
    unsigned int Tail;
    unsigned int Size;
}volatile Q_T;

int Q_Enqueue(Q_T * Q, struct entry DataEntry);

struct entry Q_Dequeue(Q_T *Q);
...
struct entry Q_Dequeue(Q_T *Q)
{
    struct entry CookerData;
    //check if queue is empty
      CookerData = Q->CookerData[Q->Head];
      Q->Head %= MAX_SIZE;
      Q->Size--;
    return CookerData;
}